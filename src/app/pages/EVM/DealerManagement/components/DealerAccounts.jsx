...
const DealerAccounts = () => {
  ...
  const fetchDealers = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${api.dealer}/api/Dealers?pageNumber=${page}&pageSize=${pageSize}`);
 ...
 const handleSave = async () => {
  try {
    const res = await fetch(`${api.dealer}/api/Dealers/${selectedDealer.dealerId}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...selectedDealer, status: selectedDealer.status?.toLowerCase() }),
    });
  ...
  const handleCreate = async (newDealer) => {
    try {
      const res = await fetch(`${api.dealer}/api/Dealers`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...newDealer, status: newDealer.status?.toLowerCase() }),
      });
      if (res.ok) {
        const created = await res.json();
        const dealer = created.data; // lấy đúng dealer object từ API
        if (!dealer.dealerId) {
  ...
  const SkeletonRow = () => (
    <tr className="animate-pulse">
      ...
      <td className="px-6 py-4"><Loader2 className="w-4 h-4 text-gray-400 animate-spin" /></td>
    </tr>
  );
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="flex space-x-3">
          <button className="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700" onClick={() => setShowCreateModal(true)}>
            + Create Account
          </button>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full border border-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-6 py-3 text-left text-sm font-medium text-gray-500">Dealer Code</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            ...
            {!loading && dealers.map((dealer, index) => (
              <tr key={dealer.dealerId || index}>
                <td className="px-6 py-4 text-sm font-medium text-gray-900">{dealer.dealerCode}</td>
                <td className="px-6 py-4 text-sm font-medium text-gray-900">{dealer.name}</td>
                <td className="px-6 py-4 text-sm text-gray-600">{dealer.region}</td>
                <td className="px-6 py-4 text-sm text-gray-600">{dealer.contactEmail}<br />{dealer.contactPhone}</td>
                <td className="px-6 py-4">
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(dealer.status)}`}>{dealer.status || "n/a"}</span>
                </td>
                <td className="px-6 py-4 text-sm space-x-3 flex items-center">
                  <button className="text-gray-500 hover:text-gray-700" onClick={() => handleEdit(dealer)}><Pencil className="w-4 h-4" /></button>
                </td>
              </tr>
            ))}
            {!loading && dealers.length === 0 && (
              <tr>
                <td colSpan="6" className="px-6 py-4 text-center text-gray-500">No dealer data available</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      {showModal && (
        <UpdateDealerModal dealer={selectedDealer} setDealer={setSelectedDealer} onClose={() => setShowModal(false)} onSave={handleSave} />
      )}
      {showCreateModal && (
        <CreateDealerModal onClose={() => setShowCreateModal(false)} onCreate={handleCreate} />
      )}
    </div>
  );
};
